...
class ScanParams:
    ...
    workers: int = 5
    tail_window: int = 100
    output_dir: Path = Path("data/scan_r/B")
    dpi: int = 300
    progress: bool = True
    warmup: bool = True
...
def save_plot(params: ScanParams, summaries: list[dict[str, float | int | str]]) -> Path:
    ...
    ax.grid(True, linestyle="--", linewidth=0.5, alpha=0.4)
    ax.set_title("Model B: cooperation vs r")
    fig.tight_layout()

    path = params.output_dir / "B_cooperation.png"
...
def run_scan(params: ScanParams) -> tuple[Path, Path, Path, list[Path]]:
    ...
    if params.workers == 1:
        ...
        if params.progress and tqdm is not None:
            iterator = tqdm(tasks, desc="Model B", unit="run", dynamic_ncols=True)
        ...
        with ProcessPoolExecutor(max_workers=max_workers) as executor:
            ...
            if params.progress and tqdm is not None:
                iterator = tqdm(iterator, total=len(futures), desc="Model B", unit="run", dynamic_ncols=True)
    ...
    raw_csv = params.output_dir / "B_runs.csv"
    summary_csv = params.output_dir / "B_summary.csv"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Scan r values for Model B.")
