...
class ScanParams:
    ...
    workers: int = 5
    tail_window: int = 100
    output_dir: Path = Path("data/payoff_r/A")
    dpi: int = 300
    progress: bool = True
    warmup: bool = True
...
def warmup_numba() -> None:
    simulate_model_a_payoff(4, 2, 2.5, 1.0, 0.5, 1.0, 0.3, 1.0 / 3.0, 1.0 / 3.0, 1.0 / 3.0, 1, 123)
...
def simulate_one(params: ScanParams, r_value: float, seed: int, chi: float) -> dict[str, float | int | str]:
    ...
    system_payoff = simulate_model_a_payoff(
        int(params.L),
        int(params.T),
        float(r_value),
        float(params.beta_F),
...
def plot_summary(params: ScanParams, summaries: list[dict[str, float | int | str]]) -> list[Path]:
    ...
    ax.set_title(r"Model A: system payoff vs r for fixed $\chi$")
    ax.grid(True, linestyle="--", linewidth=0.5, alpha=0.4)
    ax.legend(title=r"$\chi$", loc="best")
    fig.tight_layout()
    return save_one_figure(fig, params.output_dir / "A_system_payoff_vs_r", params.dpi)
...
def run_scan(params: ScanParams) -> tuple[Path, Path, list[Path]]:
    ...
    if params.workers == 1:
        ...
        if params.progress and tqdm is not None:
            iterator = tqdm(tasks, desc="A payoff r scan", unit="run", dynamic_ncols=True)
        ...
        with ProcessPoolExecutor(max_workers=params.workers) as executor:
            ...
            if params.progress and tqdm is not None:
                iterator = tqdm(iterator, total=len(futures), desc="A payoff r scan", unit="run", dynamic_ncols=True)
    ...
    raw_csv = params.output_dir / "A_payoff_r_runs.csv"
    summary_csv = params.output_dir / "A_payoff_r_summary.csv"
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Scan Model A system average payoff versus r.")
