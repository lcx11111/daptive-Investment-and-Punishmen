...
# ============================================================
# Double Q-learning Model E eta_c-gamma_c heatmap - single GPU CUDA version
#
...
#   R_c = M_c, R_a = M_a
#
# The heatmap scans the contribution Q-table learning rate eta_c and
# discount factor gamma_c. Punishment Q-table eta_a/gamma_a are fixed.
#
#   python a_c.py --gpu-batch 1024 --threads-per-block 256
# ============================================================
...
class ScanParams:
    ...
    fixed_alpha_a: float = 0.8
    fixed_gamma_a: float = 0.8
    ...
    output_dir: Path = Path("data/E_eta_c_gamma_c_heatmap")
    dpi: int = 600
    progress: bool = True
    warmup: bool = True
...
def validate_scan_params(params: ScanParams) -> tuple[float, float, float]:
    ...
    if not (0.0 <= params.fixed_alpha_a <= 1.0):
        raise ValueError(f"Invalid fixed_alpha_a={params.fixed_alpha_a}; expected 0 <= fixed_alpha_a <= 1.")
    if not (0.0 <= params.fixed_gamma_a <= 1.0):
        raise ValueError(f"Invalid fixed_gamma_a={params.fixed_gamma_a}; expected 0 <= fixed_gamma_a <= 1.")
    if params.runs <= 0:
...
def scan_tag(params: ScanParams) -> str:
    return (
        ...
        f"_fixedEtaA{format_value(params.fixed_alpha_a)}_fixedGammaA{format_value(params.fixed_gamma_a)}"
...
def warmup_gpu(params: ScanParams) -> None:
    ...
    tasks = [
        (0, 0, 0, small.alpha_min, small.gamma_min, small.fixed_alpha_a, small.fixed_gamma_a),
        (1, 1, 0, small.alpha_max, small.gamma_max, small.fixed_alpha_a, small.fixed_gamma_a),
    ]
...
def run_scan(params: ScanParams) -> tuple[Path, Path, list[Path]]:
            ...
            for run_idx in range(params.runs):
                tasks.append(
                    (
                        ...
                        float(alpha),
                        float(gamma),
                        float(params.fixed_alpha_a),
                        float(params.fixed_gamma_a),
                    )
                )
    ...
    if params.progress and tqdm is not None:
        iterator = tqdm(iterator, total=len(batches), desc="Model E eta_c-gamma_c scan", unit="batch", dynamic_ncols=True)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="GPU heatmap for double Q-learning Model E over eta_c and gamma_c.")
    ...
    parser.add_argument("--eta-c-min", "--alpha-c-min", "--alpha-min", dest="alpha_min", type=float, default=CONFIG_SCAN.alpha_min)
    parser.add_argument("--eta-c-max", "--alpha-c-max", "--alpha-max", dest="alpha_max", type=float, default=CONFIG_SCAN.alpha_max)
    parser.add_argument("--eta-c-points", "--alpha-c-points", "--alpha-points", dest="alpha_points", type=int, default=CONFIG_SCAN.alpha_points)
    parser.add_argument("--gamma-c-min", "--gamma-min", dest="gamma_min", type=float, default=CONFIG_SCAN.gamma_min)
    parser.add_argument("--gamma-c-max", "--gamma-max", dest="gamma_max", type=float, default=CONFIG_SCAN.gamma_max)
    parser.add_argument("--gamma-c-points", "--gamma-points", dest="gamma_points", type=int, default=CONFIG_SCAN.gamma_points)
    parser.add_argument("--K", type=float, default=CONFIG_SCAN.K)
    parser.add_argument("--fixed-eta-a", "--fixed-alpha-a", dest="fixed_alpha_a", type=float, default=CONFIG_SCAN.fixed_alpha_a)
    parser.add_argument("--fixed-gamma-a", dest="fixed_gamma_a", type=float, default=CONFIG_SCAN.fixed_gamma_a)
...
def params_from_args(args: argparse.Namespace) -> ScanParams:
    return replace(
        ...
        K=args.K,
        fixed_alpha_a=args.fixed_alpha_a,
        fixed_gamma_a=args.fixed_gamma_a,
