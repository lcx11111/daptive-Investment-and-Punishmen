...
class ScanParams:
    ...
    # Model E in double_Q.tex: no successful-neighbor reference reward.
    w_c: float = 0.0
    w_a: float = 0.0
...
def q_update_and_copy_kernel(
    ...
    # Contribution Q-table update: R_c = M_c + w_c S_c.
    # S_c uses the current contribution action of the highest-payoff
    # cooperative neighbor whose payoff is higher than the focal player.
    if strategy == C or strategy == P:
        s0 = state_cp_old[tid]
        ...
        best_c_idx = -1
        best_c_payoff = payoff[tid]
        if (z_old[idx_up] == C or z_old[idx_up] == P) and payoff[idx_up] > best_c_payoff:
            best_c_payoff = payoff[idx_up]
            best_c_idx = idx_up
        if (z_old[idx_down] == C or z_old[idx_down] == P) and payoff[idx_down] > best_c_payoff:
            best_c_payoff = payoff[idx_down]
            best_c_idx = idx_down
        if (z_old[idx_left] == C or z_old[idx_left] == P) and payoff[idx_left] > best_c_payoff:
            best_c_payoff = payoff[idx_left]
            best_c_idx = idx_left
        if (z_old[idx_right] == C or z_old[idx_right] == P) and payoff[idx_right] > best_c_payoff:
            best_c_payoff = payoff[idx_right]
            best_c_idx = idx_right

        social_c = 0.0
        if best_c_idx >= 0:
            social_c = 1.0 - abs(c_value - contribution_value[best_c_idx]) / 0.9

        reward_c = match_c + w_c * social_c
        target = reward_c + gamma_c * next_max
        q_c[_q_c_index(tid, s0, a0)] = old_q + alpha_c * (target - old_q)

    # Punishment Q-table update: R_a = M_a + w_a S_a.
    ...
    if strategy == P:
        ...
        best_a_idx = -1
        best_a_payoff = payoff[tid]
        if z_old[idx_up] == P and payoff[idx_up] > best_a_payoff:
            best_a_payoff = payoff[idx_up]
            best_a_idx = idx_up
        if z_old[idx_down] == P and payoff[idx_down] > best_a_payoff:
            best_a_payoff = payoff[idx_down]
            best_a_idx = idx_down
        if z_old[idx_left] == P and payoff[idx_left] > best_a_payoff:
            best_a_payoff = payoff[idx_left]
            best_a_idx = idx_left
        if z_old[idx_right] == P and payoff[idx_right] > best_a_payoff:
            best_a_payoff = payoff[idx_right]
            best_a_idx = idx_right

        social_a = 0.0
        if best_a_idx >= 0:
            social_a = 1.0 - abs(a_value - punishment_value[best_a_idx]) / 0.4

        reward_a = match_a + w_a * social_a
...
def validate_scan_params(params: ScanParams) -> tuple[float, float, float]:
    ...
    if not (0.0 <= params.w_c <= 1.0 and 0.0 <= params.w_a <= 1.0):
        raise ValueError(f"Invalid weights: w_c={params.w_c}, w_a={params.w_a}.")
    if params.runs <= 0:
