...
@njit(cache=True)
def assign_random_model_B_actions_numba(
    z: np.ndarray,
    contribution_value: np.ndarray,
    punishment_value: np.ndarray,
    contribution_actions: np.ndarray,
    ...
    """Model B: random costs each round.

    C/P draw contribution costs uniformly from A_c in every round.
    P draws punishment costs uniformly from A_a in every round.
    D neither contributes nor punishes.
    """
    L = z.shape[0]
    n_c_actions = contribution_actions.shape[0]
    n_a_actions = punishment_actions.shape[0]
...
def simulate_model_E_metric_evolution_numba(
    ...
    The row at t=0 is initialized to zero so payoff-like
    statistics start from the initial time point.
...
@njit(cache=True)
def simulate_model_B_metric_evolution_numba(
    L: int,
    T: int,
    r_value: float,
    beta_F: float,
    ...
    """Return Model B metric curves for t=0,...,T.

    The payoff-like metrics at t=0 are initialized to zero. The first
    realized game-round statistics are stored at t=1. In every realized
    round, contribution and punishment costs are redrawn uniformly
    from their respective action spaces. Column 3 stores defector density:
    t=0 is the initial density, and t>=1 is recorded after strategy updating.
    """
    np.random.seed(seed)
    ...
    for step in range(1, T + 1):
        assign_random_model_B_actions_numba(
            z,
            contribution_value,
            punishment_value,
            contribution_actions,
...
def warmup_metric_compare_numba() -> None:
    ...
    simulate_model_B_metric_evolution_numba(
        4, 2, 2.5, 1.0, 0.5,
...
def build_metric_compare_tag(
    ...
    a_tag = "-".join(format_value(x) for x in punishment_actions)
    return (
        f"modelB_vs_E_metrics_logt_t0_L{params.L}_T{params.T}_r{format_value(params.r)}"
        f"_runs{params.runs}_seed{params.seed_start}"
        f"_bF{format_value(params.beta_F)}_K{format_value(params.K)}"
        f"_aC{format_value(params.alpha_c)}_gC{format_value(params.gamma_c)}"
        f"_aA{format_value(params.alpha_a)}_gA{format_value(params.gamma_a)}"
...
def build_metric_compare_figure_tag(params: EvolutionParams) -> str:
    return (
        f"modelB_vs_E_L{params.L}_T{params.T}_r{format_value(params.r)}"
        f"_runs{params.runs}_seed{params.seed_start}"
    )
...
def plot_metric_compare_panel(
    ...
    *,
    ...
    ax.plot(t_plus_1, model_B_mean[:, metric_idx], linestyle="-", label="Model B")
    ax.plot(
        t_plus_1,
...
def save_metric_compare_outputs(
    params: EvolutionParams,
    model_B_mean: np.ndarray,
    model_B_std: np.ndarray,
    model_B_sem: np.ndarray,
    model_E_mean: np.ndarray,
    ...
    """Save one CSV plus four Model B vs E evolution plots.
    ...
    """
    params.output_dir.mkdir(parents=True, exist_ok=True)
    tag = build_metric_compare_tag(params, contribution_actions, punishment_actions)
    ...
    header = (
        "t,t_plus_1,"
        "mean_B_net_payoff,mean_E_net_payoff,"
        "mean_B_punisher_outlay,mean_E_punisher_outlay,"
        "mean_B_defector_fine,mean_E_defector_fine,"
        "mean_B_defector_density,mean_E_defector_density,"
        "std_B_net_payoff,std_E_net_payoff,"
        "std_B_punisher_outlay,std_E_punisher_outlay,"
        "std_B_defector_fine,std_E_defector_fine,"
        "std_B_defector_density,std_E_defector_density,"
        "sem_B_net_payoff,sem_E_net_payoff,"
        "sem_B_punisher_outlay,sem_E_punisher_outlay,"
        "sem_B_defector_fine,sem_E_defector_fine,"
        "sem_B_defector_density,sem_E_defector_density"
    )
    np.savetxt(
        csv_path,
        csv_data,
    ...
    figure_tag = build_metric_compare_figure_tag(params)
    figure_paths: list[Path] = []
...
def run_metric_compare(params: EvolutionParams) -> tuple[Path, list[Path]]:
    ...
    curves_B = np.empty((int(params.runs), int(params.T) + 1, 4), dtype=np.float64)
    curves_E = np.empty((int(params.runs), int(params.T) + 1, 4), dtype=np.float64)

    for run_idx in range(int(params.runs)):
        seed = int(params.seed_start) + run_idx
        print(f"running Model B metric evolution {run_idx + 1}/{params.runs}, seed={seed}...")
        start = time.perf_counter()
        curves_B[run_idx] = simulate_model_B_metric_evolution_numba(
            int(params.L),
            int(params.T),
            float(params.r),
            float(params.beta_F),
        ...
        print(f"Model B run {run_idx + 1}/{params.runs} done: {time.perf_counter() - start:.2f}s")
    ...
    B_mean, B_std, B_sem = summarize_runs(curves_B)
...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(
        description="Save Model B vs E metric evolution curves plus defector-density evolution on a logarithmic t+1 axis."
    )
